...
def test_upload_document_success(client, mock_s3, mock_rag_ingest):
    data = _upload(client)

    assert data["file_name"] == "test.pdf"
    assert data["user_id"] == str(MVP_USER_ID)
    mock_s3.upload_fileobj.assert_called_once()
    mock_rag_ingest.assert_called_once()


# ── GET /documents ────────────────────────────────────────────────────────────


def test_list_documents(client, mock_s3, mock_rag_ingest):
...
def test_delete_document_success(client, mock_s3, mock_rag_ingest):
